...
PetscErrorCode updateIntermediateVelocity_fs(DM da, Vec u_vec, Vec v_vec,
...
int main(int argc, char **argv) {
  ...
  DMDACreate2d(PETSC_COMM_WORLD, DM_BOUNDARY_NONE, DM_BOUNDARY_NONE,
               DMDA_STENCIL_STAR, Nx, Ny, PETSC_DECIDE, PETSC_DECIDE, 1, 1,
               NULL, NULL, &da);
...
void setVelocityBoundaryConditions(DM da, Vec u_vec, Vec v_vec) {
  ...
  for (PetscInt i = info.xs; i < info.xs + info.xm; i++) {
    ...
    gamma_top = (vArray[i][info.ys + info.ym - 2] - yc[NY - 2]) /
                (yc[NY - 2] - yc[NY - 3]);
    uArray[i][info.ys + info.ym - 1] =
        (U_WALL - (1 - gamma_top) * uArray[i][info.ys + info.ym - 2]) /
        gamma_top;
    vArray[i][info.ys + info.ym - 2] = 0.0;
    // Bottom boundary: u=0, v=0
    gamma_bottom = (vArray[i][info.ys] - yc[0]) / (yc[1] - yc[0]);
    uArray[i][info.ys] =
        -gamma_bottom / (1 - gamma_bottom) * uArray[i][info.ys + 1];
  ...
  for (PetscInt j = info.ys; j < info.ys + info.ym; j++) {
    ...
    vArray[info.xs][j] =
        -gamma_west / (1 - gamma_west) * vArray[info.xs + 1][j];
    // Right boundary: u=0, v=0
    gamma_east = (uArray[info.xs + info.xm - 2][j] - xc[NX - 2]) /
                 (xc[NX - 2] - xc[NX - 3]);
    uArray[info.xs + info.xm - 2][j] = 0.0;
    vArray[info.xs + info.xm - 1][j] =
        -(1 - gamma_east) / gamma_east * vArray[info.xs + info.xm - 2][j];
  }
