...
PetscErrorCode  updateIntermediateVelocity_fs(DM da, Vec u_vec, Vec v_vec, Vec p_vec);
PetscErrorCode  updatePressureField(DM da, Vec u_vec, Vec v_vec, Vec p_vec);
PetscErrorCode  updateVelocityField(DM da, Vec u_vec, Vec v_vec, Vec p_vec);
...
PetscErrorCode  setVelocityBoundaryConditions(DM da, Vec u_vec, Vec v_vec);
...
int main(int argc, char **argv) {
    ...
    for (int iter = 0; iter < MAX_ITER; iter++) {
        ...
        updateIntermediateVelocity_fs(da, u_vec, v_vec, p_vec);
        updatePressureField(da, u_vec, v_vec, p_vec);
        updateVelocityField(da, u_vec, v_vec, p_vec);
...
PetscErrorCode updateIntermediateVelocity_fs(DM da, Vec u_vec, Vec v_vec, Vec p_vec) {
    ...
    DMGetLocalVector(da, &localP);

    DMDAVecGetArray(da, localU, &uLocal);
    DMDAVecGetArray(da, localV, &vLocal);
    DMDAVecGetArray(da, localP, &pLocal);
    ...
    rhsSolutionX_ = (double **)malloc(NX * sizeof(double *));
    rhsSolutionX = (double **)malloc(NX * sizeof(double *));
    rhsSolutionY_ = (double **)malloc(NX * sizeof(double *));
    rhsSolutionY = (double **)malloc(NX * sizeof(double *));
    ...
    for (int i = 0; i < NX; i++) {
        ...
        rhsSolutionX_[i] = (double *)malloc(NY * sizeof(double));
        rhsSolutionX[i] = (double *)malloc(NY * sizeof(double));
        rhsSolutionY_[i] = (double *)malloc(NY * sizeof(double));
        rhsSolutionY[i] = (double *)malloc(NY * sizeof(double));
    }
    ...
    for (int i = 1; i < NX - 1; ++i) {
        for (int j = 1; j < NY - 1; ++j) {
            uLocal[i][j] = u[i][j] + rhsSolutionX[i][j];
            vLocal[i][j] = v[i][j] + rhsSolutionY[i][j];
        }
    }

    // Restore arrays
    DMDAVecRestoreArray(da, localU, &uLocal);
    DMDAVecRestoreArray(da, localV, &vLocal);
    DMDAVecRestoreArray(da, localP, &pLocal);

    DMRestoreLocalVector(da, &localU);
    DMRestoreLocalVector(da, &localV);
    DMRestoreLocalVector(da, &localP);
...
PetscErrorCode updatePressureField(DM da, Vec u_vec, Vec v_vec, Vec p_vec) {
    PetscErrorCode ierr;
    PetscInt       i, j, M, N, mstart, nstart, mend, nend;

    DMDAGetInfo(da, NULL, &M, &N, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
...
void updateVelocityField(DM da, Vec u_vec, Vec v_vec, Vec p_vec, PetscReal u_hat[][NY], PetscReal v_hat[][NY]) {
    // Loop through local elements
    for (PetscInt j = info.ys; j < info.ys + info.ym; j++) {
        for (PetscInt i = info.xs; i < info.xs + info.xm; i++) {
            u_vec[j][i] = u_hat[j][i] - DT * (p_vec[j][i] - p_vec[j][i - 1]) / (xc[i] - xc[i-1]);
            v_vec[j][i] = v_hat[j][i] - DT * (p_vec[j][i] - p_vec[j - 1][i]) / (yc[j] - yc[j-1]);
        }
    }
}
...
void setPressureBoundaryConditions(DM da, Vec p_vec) {
    ...
    PetscScalar **pLocal;
    ierr = DMDAVecGetArray(da, p_vec, &pLocal);
    CHKERRABORT(PETSC_COMM_WORLD, ierr);
...
PetscErrorCode setVelocityBoundaryConditions(DM da, Vec u_vec, Vec v_vec) {
    ...
    // Get local vectors associated with the DMDA
    Vec uLocal, vLocal;
    DMCreateLocalVector(da, &uLocal);
    DMCreateLocalVector(da, &vLocal);

    // Get ghosted arrays for u and v
    PetscScalar **uArray, **vArray;
    DMDAVecGetArray(da, u_vec, &uArray);
    DMDAVecGetArray(da, v_vec, &vArray);

    // Get local arrays for u and v
    PetscScalar **uLocalArray, **vLocalArray;
    DMDAVecGetArray(da, uLocal, &uLocalArray);
    DMDAVecGetArray(da, vLocal, &vLocalArray);
    ...
    for (PetscInt i = info.xs; i < info.xs + info.xm; i++) {
        ...
        uLocalArray[i][info.ys + info.ym - 1] = (U_WALL - (1 - gamma_top) * uLocalArray[i][info.ys + info.ym - 2]) / gamma_top;
        vLocalArray[i][info.ys + info.ym - 2] = 0.0;
        // Bottom boundary: u=0, v=0
        gamma_bottom = (vArray[i][info.ys] - yc[0]) / (yc[1] - yc[0]);
        uLocalArray[i][info.ys] = -gamma_bottom / (1 - gamma_bottom) * uLocalArray[i][info.ys + 1];
        vLocalArray[i][info.ys] = 0.0;
    }

    for (PetscInt j = info.ys; j < info.ys + info.ym; j++) {
        // Left boundary: u=0, v=0
        gamma_west = (uArray[info.xs][j] - xc[0]) / (xc[1] - xc[0]);
        uLocalArray[info.xs][j] = 0.0;
        vLocalArray[info.xs][j] = -gamma_west / (1 - gamma_west) * vLocalArray[info.xs + 1][j];
        // Right boundary: u=0, v=0
        gamma_east = (uArray[info.xs + info.xm - 2][j] - xc[NX-2]) / (xc[NX-2] - xc[NX-3]);
        uLocalArray[info.xs + info.xm - 2][j] = 0.0;
        vLocalArray[info.xs + info.xm - 1][j] = -(1 - gamma_east) / gamma_east * vLocalArray[info.xs + info.xm - 2][j];
    }

    // Restore arrays
    DMDAVecRestoreArray(da, u_vec, &uArray);
    DMDAVecRestoreArray(da, v_vec, &vArray);
    DMDAVecRestoreArray(da, uLocal, &uLocalArray);
    DMDAVecRestoreArray(da, vLocal, &vLocalArray);
